...
addCart.forEach((el) => {
  el.addEventListener("click", (e) => {
    ...
    const price_Item =
      e.target.previousElementSibling.previousElementSibling.firstChild.data;
    const tittle_Item =
      e.target.previousElementSibling.querySelector("h3").firstChild.data;
...
const btnRemoveOrder = (event) => {
  ...
  const nameOrderItem =
    event.target.previousElementSibling.previousElementSibling
      .previousElementSibling.previousElementSibling.firstChild.data;

  listOrder.removeChild(removeOrder);
  addCart.forEach((btnCart) => {
    if (
      nameOrderItem ==
      btnCart.previousElementSibling.firstChild.nextElementSibling.firstChild
        .data
    ) {
...
const checkOutPrint = () => {
  ...
  document.querySelector("#subtotal h4").textContent = `$${String(
    subTotal
  ).substring(0, 5)}`;
  document.querySelectorAll("#total h3")[1].textContent = `$${String(
    total
  ).substring(0, 5)}`;
};
